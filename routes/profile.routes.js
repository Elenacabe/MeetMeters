...
router.get("/edit/:_id", isLoggedIn, (req, res) => {
  ...
  User.findById(_id)
    .then((user) =>
      res.render(`/Profile/details/${_id}`, {
        user,
        isAdmin: req.session.currentUser.role === "ADMIN",
        profile:
          req.session.currentUser._id === _Id ||
          req.session.currentUser.role === "ADMIN",
      })
    )
    .catch((err) => next(err));
...
router.post(
  ...
  (req, res, next) => {
    ...
    User.findByIdAndUpdate(_id, { username, about, avatar })
...
router.post(
  ...
  (req, res, next) => {
    ...
    User.findByIdAndUpdate(_id, { role })
      .then(() => res.redirect(`/profile/list`))
      .catch((err) => next(err));
  }
);
