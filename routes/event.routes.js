...
router.post(
  "/eventCreation",
  uploaderMiddleware.single("cover"),
  isLoggedIn,
  checkRole("ADMIN", "GUIDE"),
  (req, res, next) => {
    const { path: cover } = req.file;
    const { name, description, startDate, endDate, latitude, longitude, type } =
      req.body;
...
router.post(
  ...
  (req, res, next) => {
    ...
    Event.findByIdAndUpdate(_id, { cover, name, description, location, owner })
...
router.post("/:event_id/createComment", isLoggedIn, (req, res, next) => {
  ...
  Comment.create({ comment, author: author_id })
    .then((commentCreated) =>
